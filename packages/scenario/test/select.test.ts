...
describe.each<GetSelectFromRepeatFormOptions>([
	...
	'Select - JavaRosa ports (substituting absolute body references: $substituteAbsoluteBodyReferences)',
	({ substituteAbsoluteBodyReferences }) => {
		...
		const getSelectFromRepeatForm = (predicate = '') => {
			const repeatValueInputRef = substituteAbsoluteBodyReferences ? '/data/repeat/value' : 'value';
			const repeatLabelInputRef = substituteAbsoluteBodyReferences ? '/data/repeat/label' : 'label';
