...
abstract class NumberExpressionEvaluator<ConstValue extends number | null = null>
  ...
  constructor(protected readonly constValue: ConstValue) {
    this.constEvaluation = constValue == null
      ? null
      : new NumberEvaluation(constValue);
  }
...
abstract class StringExpressionEvaluator<ConstValue extends string | null = null>
  ...
  constructor(protected readonly constValue: ConstValue) {
    this.constEvaluation = constValue == null
      ? null
      : new StringEvaluation(constValue);
  }
...
export abstract class LocationPathExpressionEvaluator
  implements ExpressionEvaluator {
...
type BinaryExpressionEvaluation =
  | BooleanEvaluation
  | NumberEvaluation
  | LocationPathEvaluation
  ;

interface TypedExpressionEvaluator<
  Evaluated extends Evaluation
...
> implements TypedExpressionEvaluator<Evaluated> {
  ...
  constructor(readonly syntaxNode: Node) {
    ...
    this.lhs = createExpression(
      lhsNode
    ) as TypedExpressionEvaluator<Evaluated>;
    this.rhs = createExpression(
      rhsNode
    ) as TypedExpressionEvaluator<Evaluated>;
  }
...
> extends BinaryExpression<Node, BooleanEvaluation> {
...
> extends BinaryExpression<Node, NumberEvaluation> {
...
class UnionExpression extends LocationPathExpressionEvaluator {
  ...
  evaluateNodes(context: EvaluationContext): Iterable<Node> {
    ...
    return sortDocumentOrder(
      distinct(
        chain(lhs.nodes, rhs.nodes)
      )
    );
  }
}
...
class FunctionCallExpression implements ExpressionEvaluator {
  ...
  readonly argumentExpressions: ReadonlyArray<ExpressionEvaluator>;
  ...
  constructor(readonly syntaxNode: FunctionCallNode) {
    ...
    this.argumentExpressions = argumentNodes.map((syntaxNode) => {
      return createExpression(syntaxNode);
    });
  }
      ...
      for (const predicateNode of step.predicates) {
        ...
        for (const self of ctx) {
          ...
          if (predicateResult.type === 'NUMBER') {
            const positionPredicate = predicateResult.toNumber();
        ...
        ctx = new LocationPathEvaluation(ctx, filteredNodes);
      }
    }
...
class FilterPathExpression
  extends LocationPathExpression
  implements ExpressionEvaluator {

  readonly filterExpression: LocationPathExpressionEvaluator;
  readonly hasSteps: boolean;

  constructor(readonly syntaxNode: FilterPathExprNode) {
  ...
  evaluateNodes(context: EvaluationContext): Iterable<Node> {
    if (this.hasSteps) {
      ...
      if (filterContextResults.type !== 'NODE') {
        throw 'todo not a node-set context';
      }

      const nodes = Array.from(filterContextResults.nodes);

      if (nodes.length > 1) {
        throw 'todo multiple nodes context';
      }

      const [contextNode] = nodes;

      if (contextNode == null) {
        return nodes;
      }

      const filterExpressionContext = new LocationPathEvaluation(context, nodes);

      return super.evaluateNodes(filterExpressionContext);
    }
...
class AbsoluteLocationPathExpression
  extends LocationPathExpression
  implements ExpressionEvaluator {

  constructor(readonly syntaxNode: AbsoluteLocationPathNode) {
...
class RelativeLocationPathExpression
  extends LocationPathExpression
  implements ExpressionEvaluator {

  constructor(readonly syntaxNode: RelativeLocationPathNode) {
    const { text } = syntaxNode;
    const isSelf = text === '.';
