...
const axisTestStep = (
  ...
  switch (axisTestNode.type) {
    ...
    case 'prefixed_wildcard_name_test': {
      ...
      return new QualifiedWildcardTestStep(
        axisType,
        prefixNode.text,
        predicateNodes
      );
    ...
    case 'unprefixed_wildcard_name_test':
      return new UnqualifiedWildcardTestStep(
        axisType,
        predicateNodes
      );
    ...
    case 'processing_instruction_name_test': {
      ...
      const { text } = nameNode;
      const procssingInstructionName = text.substring(1, text.length - 1);
...
const pathExprContextStep = (
  syntaxNode: PathExprContextNode
): PathExprContextStep => {
  switch (syntaxNode.type) {
...
export type PathExprSteps = readonly [
  context: PathExprContextStep,
  ...steps: ContextualizedStep[],
];

export const pathExprSteps = (
  syntaxNode: AnyLocationPathExprNode
): PathExprSteps => {
