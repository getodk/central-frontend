...
export const countSelected = new NumberFunction(
  ...
  (context, [listExpression]): number => {
    const string = trimXPathWhitespace(
      listExpression!.evaluate(context).toString()
    );
...
export const selected = new BooleanFunction(
  ...
  (context, [listExpression, valueExpression]): boolean => {
    const list = whitespaceSeparatedList(
      listExpression!.evaluate(context).toString()
    );
    ...
    const value = trimXPathWhitespace(
      valueExpression!.evaluate(context).toString()
    );
