...
export const isAttributeNode = (
	node: Node
): node is Attr => (
	node.nodeType === ATTRIBUTE_NODE
);

export const isNamespaceAttribute = (
  attr: Attr
): attr is NamespaceAttribute => attr.name === 'xmlns' || attr.prefix === 'xmlns';

export const isNamespaceNode = (node: Node): node is NamespaceAttribute => (
  isAttributeNode(node) &&
  (node.name === 'xmlns' || node.prefix === 'xmlns')
);

export const isCDataSection = (
  node: Node
...
export const isProcessingInstructionNode = (
	node: Node
