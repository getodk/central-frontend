...
export type FilteredTreeWalker<
  Filter extends TreeWalkerFilter = TreeWalkerFilter
> =
  & TreeWalker
  & { readonly whatToShow: TreeWalkerFilters[Filter] }
  ;

export const getTreeWalker = <
  Filter extends TreeWalkerFilter = 'ANY'
>(
  ...
  return contextDocument.createTreeWalker(
    contextNode,
    flag
  ) as FilteredTreeWalker<Filter>;
};

export type FilteredTreeWalkers = {
  readonly [Filter in TreeWalkerFilter]: FilteredTreeWalker<
    Filter
  >;
};
