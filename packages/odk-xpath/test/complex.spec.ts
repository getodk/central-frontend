...
describe('some complex examples', () => {
  ...
  [
    { expression: 'concat("uuid:", uuid())', match: /uuid:[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}/ },
    ...
    { expression: 'format-date-time(date-time(decimal-date-time("2003-03-12") + 280), "%b %e, %Y")', match: /^Dec 17, 2003$/ },
    ...
    { expression: "if(selected(/model/instance[1]/pregnancy/group_lmp/lmp_method, 'date'), /model/instance[1]/pregnancy/group_lmp/lmp_date, 'testing')", match: /testing/ },
    { expression: "if(selected(/model/instance[1]/pregnancy/group_lmp/lmp_method, 'date'), /model/instance[1]/pregnancy/group_lmp/lmp_date, concat('testing', '1', '2', '3', '...'))", match: /testing/ },
    { expression: "if(selected(/model/instance[1]/pregnancy/group_lmp/lmp_method, 'date'), /model/instance[1]/pregnancy/group_lmp/lmp_date, date-time(0))", match: SIMPLE_DATE_OR_DATE_TIME_MATCH },
    { expression: "if(selected(/model/instance[1]/pregnancy/group_lmp/lmp_method, 'date'), /model/instance[1]/pregnancy/group_lmp/lmp_date, date-time(decimal-date-time(today() - 60)))", match: SIMPLE_DATE_MATCH },
    { expression: "if(selected(/model/instance[1]/pregnancy/group_lmp/lmp_method ,'date'), /model/instance[1]/pregnancy/group_lmp/lmp_date ,date-time(decimal-date-time(today()- 60 )))", match: SIMPLE_DATE_MATCH },
    ...
    { expression: '((true() or false()) and (false() or true())) and (false() or true())', expected: true },
    { expression: '((true() or false()) and (false() or false())) and (false() or true())', expected: false },
    ...
    it('should convert "' + expression + '" to match "' + (expected ?? match ?? predicate) + '"', () => {
      if (predicate != null) {
