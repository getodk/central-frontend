...
describe('#uuid()', () => {
  ...
  it('should provide an RFC 4122 version 4 compliant UUID string', () => {
    testContext.assertStringMatches('uuid()',
  ...
  describe('referencing nodesets', () => {
    ...
    [
      { expression: 'uuid(/nonsense)' },
      { expression: 'uuid(/numbers)' },
    ].forEach(({ expression }) => {
      it.fails(`should throw an error when evaluating '${expression}' because the nodeset evaluates to NaN`, () => {
