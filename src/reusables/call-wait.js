...
is true. This composable/mixin includes the function/method callWait() to help
do so.
...
must also specify a name for the check. For example:
  ...
  callWait(
    ...
    () => {
      ...
      setAlert('info', 'It is the afternoon!');
      return true;
    },
...
A component using the mixin must define the following data property:
...
const methods = (calls) => {
  function callWait(name, call, wait) {
    if (calls(this)[name] != null) throw new Error('name is in use');
    ...
    calls(this)[name] = cancel; // eslint-disable-line no-param-reassign
    ...
    const f = () => {
      ...
      Promise.resolve(call())
        .then(success => {
          if (canceled) return;
          if (success) {
            delete calls(this)[name]; // eslint-disable-line no-param-reassign
          } else {
            count += 1;
        ...
        .catch(() => {
          ...
          if (!canceled) delete calls(this)[name];
        });
    };
  ...
  function cancelCalls() {
    for (const name of Object.keys(calls(this)))
      cancelCall.call(this, name);
  }

  return { callWait, cancelCall, cancelCalls };
};

export const useCallWait = () => {
  const calls = {};
  const ms = methods(() => calls);
  onBeforeUnmount(ms.cancelCalls);
  return ms;
};

// @vue/component
export const mixinCallWait = {
  beforeUnmount() {
    this.cancelCalls();
  },
  methods: methods(vm => vm.calls)
};
