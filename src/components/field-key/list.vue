...
<i18n>
{
  ...
  "ja": {
    ...
    "heading": [
      {
        "full": "アプリユーザーは、{collect} などのアプリケーションからデータを収集するために使用されます。アプリユーザーは通常、\"ワクチン接種者 \"のような共有の役割を表しますが、個人を表すこともあります。このプロジェクトのアプリユーザーは、このプロジェクト内でのみフォームをダウンロードして使用することができます。新規のアプリユーザーを作成しても、最初はどのフォームにもアクセスできません。各アプリユーザーがアクセスできるフォームを設定するには、{formAccess}タブを使用します。",
        "formAccess": "フォームへのアクセス"
      },
      {
        "full": "アプリユーザーは、データ収集者が複数のフォームにアクセスする必要がある場合やオフラインの場合、または複雑なフォームを使用している場合に最適です。回答者に自己申告してもらう必要がある場合や、オンラインのみのフォームの場合は、{clickHere}して他のオプションを見て下さい。",
        "clickHere": "ここをクリック"
      }
    ],
    "header": {
    ...
    "emptyTable": "アプリユーザーが登録されていません。デバイスからフォームのダウンロードとサブミットを行うには、アプリユーザーを作成しなくてはいけません。",
    "alert": {
