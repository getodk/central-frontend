...
<i18n>
{
  ...
  "ja": {
    ...
    "warnings": {
      ...
      "testing": {
        "full": "あなたはまだモバイルデバイスで{tested}し、テストサブミッションを送信していません。これは必須ではありませんが、強く推奨します。",
        "tested": "このフォームをテスト"
      }
    },
    "introduction": [
      "この下書きフォームを、公開バージョンにしようとしています。これにより、定義フォームや添付されたメディアファイルに加えた変更が確定します。",
      "既存のサブミッションされたフォームは影響を受けませんが、すべての下書きに対するテストサブミッションは削除されます。",
      "フォームの各バージョンには、ユニークなバージョン名が必要です。現在、あなたの下書きフォームには、以前に公開されたバージョンと同じバージョン名が付けられています。新しいバージョン名を設定するには、希望するバージョン名の定義フォームをアップロードするか、以下に新しいバージョン名を入力することで、サーバーが変更を行います。"
    ],
    "field": {
      "version": "バージョン"
    },
    "problem": {
      "409_6": "指定されたバージョン名は、このフォームの過去のバージョンと競合しています。新しいものに変更して再度お試しください。"
    }
  }
}
</i18n>
