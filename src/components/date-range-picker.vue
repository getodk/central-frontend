...
export default {
  ...
  computed: {
    config() {
      ...
      // flatpickr bundles the localization for en.
  ...
  watch: {
    modelValue(modelValue) {
      this.flatpickrValue = modelValue.map(dateTime => dateTime.toJSDate());
    }
  },
  methods: {
    ...
    close(selectedDates) {
      ...
      // even though this.modelValue will not change, we still need to set
      // this.flatpickrValue.
      } else if (!complete) {
