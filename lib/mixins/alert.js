...
const factories = {};
factories.blank = (type = 'danger') => ({ state: false, type, message: null, at: new Date() });

const factory = (type) => (message) => ({ state: true, type, message, at: new Date() });
...
const mixinFactory = (options = {}) => ({
  created() {
    if (options.login === true && this.$session.loggedInSinceLastPoll())
      this.alert = factories.success('You have logged in successfully.');
  }
});
