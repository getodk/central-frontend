...
import { logRequestError, requestAlertMessage } from '../util/request';
...
const onErrorResponse = (component, requestId) => (error) => {
  ...
  const message = requestAlertMessage(error, component.problemToAlert);
  component.$alert().danger(message);
  throw error;
};
  ...
  for (const entry of entries) {
    ...
    this.$http.get(url, { headers })
      .catch(error => { // eslint-disable-line no-loop-func
        ...
        logRequestError(error);
        if (firstError)
          this.$alert().danger(requestAlertMessage(error, this.problemToAlert));
