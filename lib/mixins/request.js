...
function maybeGet(options) {
  ...
  for (const entry of entries) {
    const [prop, { url, extended = false }] = entry;
  ...
  Promise.all(requests)
    .catch(error => {
      ...
      logRequestError(error);
      this.$alert().danger(alertMessage(this, error));
      this.requestId = null;
    ...
    .then(responses => {
      ...
      for (let i = 0; i < entries.length; i += 1) {
        const [prop, { transform = R.identity }] = entries[i];
        this[prop] = MaybeData.success(transform(responses[i].data));
      }
    });
}
