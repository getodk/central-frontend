...
const parseComments = (messages, key) => {
  ...
  if (lines.every(line => line === ''))
    logThenThrow({ messages, key }, 'comments are all empty');
  while (lines[0] === '') lines.shift();
  while (last(lines) === '') lines.pop();

  // Parse tags at the top of the comment.
  const result = {};
...
const processTransifexPaths = (transifexPaths, sourceMessages) => {
  ...
  // an object with a nested comment; otherwise returns `false`.
      ...
      if (previous == null) {
        ...
        if (sourceHasComment) {
          if (previous.hasComment)
            // The same @transifexKey may be specified for multiple values, but
            // only one can contribute a comment to Transifex. Inherited and
            // autogenerated comments will not result in an error.
