...
describe('FormShow', () => {
  ...
  describe('requestData reconciliation', () => {
    it('updates requestData.attachments if it is defined but formDraft is not', async () => {
      testData.extendedForms.createPast(1);
      const attachments = testData.standardFormAttachments.createPast(1).sorted();
      const app = await load('/projects/1/forms/f', { root: false }, {
        attachments: () => attachments
      });
      app.vm.$container.requestData.attachments.data.isEmpty().should.be.true();
    });

    it('updates requestData.formDraft if it is defined but attachments is not', async () => {
      testData.extendedForms.createPast(1);
      testData.extendedFormVersions.createPast(1, { draft: true });
      const app = await load('/projects/1/forms/f', { root: false }, {
        attachments: () => mockResponse.problem(404.1)
      });
      app.vm.$container.requestData.formDraft.data.isEmpty().should.be.true();
    });
  });
