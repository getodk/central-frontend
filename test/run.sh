...
# warnings; and warnings from Karma. Ignore build warnings
if awk '/Connected on socket/ {found=1; next} found' "$output" | \
	grep -F -e 'WARN LOG:' -e 'ERROR LOG:' -e 'Module Warning' -e 'WARN [web-server]:' -C5 "$output"; then
