...
class MockHttp {
  ...
  */
    ...
    const promise = this._initialPromise()
      .then(() => this._navigateAndMount()
        ...
        .finally(pollWork != null
          ...
          : noop)
        .finally(() => { this._container.http.respondWith(null); })
  ...
  // Sets up this._container.http and sets properties of the MockHttp that are
  // used within this.afterResponses(). this._initialPromise() does nothing if
  // this._previousPromise is rejected.
  async _initialPromise() {
    ...
    this._container.http.respondWith(this._http());
  }
  ...
  _http() {
    ...
    _tryBeforeAnyResponse() and _tryBeforeEachResponse() catch any error even
    though they are called within a promise chain. Those methods catch and store
    any error so that the after responses hook is able to reject the first
    promise if something unexpected happens in the second promise.
    */
