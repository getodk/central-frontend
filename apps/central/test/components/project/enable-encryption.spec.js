...
describe('ProjectEnableEncryption', () => {
  ...
  it('focuses the passphrase input', async () => {
    ...
    modal.get('input[placeholder="Encryption password *"]').should.be.focused();
  });
  ...
  it('shows a danger alert if the passphrase is too short', async () => {
    ...
    const modal = mount(ProjectEnableEncryption, mountOptions());
    await modal.get('.btn-primary').trigger('click');
    await modal.get('input[placeholder="Encryption password *"]').setValue('x');
  ...
  it('resets the modal after it is hidden', async () => {
    ...
    const modal = mount(ProjectEnableEncryption, mountOptions());
    await modal.get('.btn-primary').trigger('click');
    await modal.get('input[placeholder="Encryption password *"]').setValue('supersecret');
    await modal.setProps({ state: false });
    await modal.setProps({ state: true });
    modal.find('.info-item').exists().should.be.true;
    await modal.get('.btn-primary').trigger('click');
    modal.get('input[placeholder="Encryption password *"]').element.value.should.equal('');
  });
  ...
  describe('after a successful response', () => {
    ...
    it('hides an alert about the passphrase length', () =>
      mockHttp()
        ...
        .request(async (modal) => {
          await modal.get('.btn-primary').trigger('click');
          const passphrase = modal.get('input[placeholder="Encryption password *"]');
