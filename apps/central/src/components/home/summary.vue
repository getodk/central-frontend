      ...
      <home-summary-item v-if="currentUser.can('user.list')" to="/users" icon="user-circle">
        <template #header>
          <template v-if="!users.initiallyLoading">
            {{ $nnnnnnnnnn('home.summary:users:isArray?' + Array.isArray(users) + ':type=' + (typeof users) + ';users:' + users + ';json' + JSON.stringify(users) + ';data:' + users.data + ';', users.length, 'default') }}
          </template>
          <template v-else>
            <spinner inline/>
          </template>
